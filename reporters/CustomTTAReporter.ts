...
class CustomTTAReporter implements Reporter {
  ...
  onBegin(config: FullConfig, suite: Suite): void {
    ...
    console.log(
      "║        🎭 MY PLAYWRIGHT AUTOMATION - REAL-TIME REPORT         ║",
    );
    console.log(
      "╠════════════════════════════════════════════════════════════════╣",
    );
  ...
  onTestEnd(test: TestCase, result: TestResult): void {
    ...
    for (const attachment of result.attachments) {
      if (attachment.contentType === "image/png") {
        ...
        const name = attachment.name || `screenshot-${screenshots.length}`;
    ...
    for (const step of currentTestSteps) {
      for (const [name, screenshotPath] of stepScreenshots) {
        if (name.includes(`step-${step.stepIndex}`)) {
  ...
  private associateLogsWithSteps(
    ...
    for (const attachment of result.attachments) {
      ...
      if (logMatch && attachment.contentType === "text/plain") {
        ...
        if (testSteps[stepIndex]) {
          try {
            const logContent = fs.readFileSync(attachment.path || "", "utf-8");
            testSteps[stepIndex].consoleLogs = logContent
              .split("\n")
    ...
    if (unassignedLogs.length > 0 && testSteps.length > 0) {
      ...
      if (stepsNeedingLogs.length > 0) {
        const logsPerStep = Math.ceil(
          unassignedLogs.length / stepsNeedingLogs.length,
        );
        unassignedLogs.forEach((log, idx) => {
          const stepIdx = Math.floor(idx / logsPerStep);
          if (stepsNeedingLogs[stepIdx]) {
            stepsNeedingLogs[stepIdx].consoleLogs!.push(log);
          }
        });
      } else {
        if (testSteps[0]) {
  ...
  private async generateReport(): Promise<void> {
    ...
    // Copy videos and screenshots to assets directory
    this.copyMediaFiles(assetsDir);
...
<body><p>Redirecting to <a href="${path.basename(this.outputFile)}">latest report</a>...</p></body></html>`;
  ...
  private copyMediaFiles(assetsDir: string): void {
    for (const test of this.testResults) {
      ...
      for (const screenshot of test.screenshots) {
        if (screenshot.path && fs.existsSync(screenshot.path)) {
          ...
          try {
            ...
            screenshot.path = `assets/${fileName}`;
          } catch (e) {
            console.error(`Failed to copy screenshot: ${screenshot.path}`, e);
          }
        }
      }
      ...
      if (test.video && fs.existsSync(test.video)) {
        const fileName = `video-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.webm`;
        const destPath = path.join(assetsDir, fileName);
        try {
          fs.copyFileSync(test.video, destPath);
          test.video = `assets/${fileName}`;
        } catch (e) {
          console.error(`Failed to copy video: ${test.video}`, e);
        }
      }

      // Copy trace
      if (test.trace && fs.existsSync(test.trace)) {
        const fileName = `trace-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.zip`;
        const destPath = path.join(assetsDir, fileName);
        try {
          fs.copyFileSync(test.trace, destPath);
          test.trace = `assets/${fileName}`;
        } catch (e) {
          console.error(`Failed to copy trace: ${test.trace}`, e);
        }
      }
      ...
      for (const step of test.steps) {
        if (step.screenshot && fs.existsSync(step.screenshot)) {
          const fileName = `step-screenshot-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.png`;
          const destPath = path.join(assetsDir, fileName);
          try {
            fs.copyFileSync(step.screenshot, destPath);
            step.screenshot = `assets/${fileName}`;
          } catch (e) {
            console.error(
              `Failed to copy step screenshot: ${step.screenshot}`,
              e,
            );
          }
        }
...
<body>
    <div class="header">
        <h1>My Playwright Automation Report</h1>
        <p class="header-subtitle">Prototyping my Work- Playwright Framework</p>
    </div>
    ...
    <footer class="report-footer">
        <p>Thanks and Regards By Karthika Ramesh <a href="https://www.linkedin.com/in/karthika-ramesh-608665170/" target="_blank">LinkedIn Profile</a></p>
    </footer>
  ...
  private generateTestTable(): string {
    ...
    for (const test of this.testResults) {
      ...
      const screenshotHtml = firstScreenshot
        ? `<a href="${firstScreenshot}" class="screenshot-link" title="View Screenshot">📸 View</a>`
        : '<span style="color: #999;">-</span>';

      const videoHtml = test.video
        ? `<a href="${test.video}" class="video-link-cell" target="_blank" title="View Video">🎥 View</a>`
        : '<span style="color: #999;">-</span>';

      const traceHtml = test.trace
        ? `<a href="${test.trace}" class="trace-link-cell" target="_blank" title="View Trace">🔍 Trace</a>`
        : '<span style="color: #999;">-</span>';

      html += `
                <tr class="test-row ${statusClass}" data-test-id="${test.id}" data-tags="${tagsData}" onclick="toggleTestDetail('${test.id}')">
                    <td class="col-sno">${serialNo}</td>
                    <td class="col-suite">${test.describePath[0] || "Root"}</td>
                    <td class="col-testname"><span class="test-name-link">${this.escapeHtml(test.title)}</span></td>
                    <td class="col-author">${author}</td>
                    <td class="col-group">${testGroup}</td>
                    <td class="col-tags">${test.tags.map((tag) => `<span class="tag">${this.escapeHtml(tag)}</span>`).join("")}</td>
                    <td class="col-file">${test.location.split(":")[0]}</td>
                    <td class="col-starttime">${testStartTime.toLocaleTimeString()}</td>
                    <td class="col-endtime">${testEndTime.toLocaleTimeString()}</td>
                    ...
                    <td class="col-trace">${traceHtml}</td>
                </tr>
            `;

      html += `<tr id="detail-row-${test.id}" class="test-detail-row" style="display: none;"><td colspan="14">`;
      html += this.generateTestDetailPanel(test);
      html += `</td></tr>`;
      ...
      html += `
            <div class="detail-section">
                ...
                <div class="section-content">
                    <div class="error-box">
                        <p class="error-message">${this.escapeHtml(test.error)}</p>
                        ${
                          test.errorStack
                            ? `
                        <details class="stack-details">
                            <summary>Stack Trace</summary>
                            <pre class="stack-trace-content">${this.escapeHtml(test.errorStack)}</pre>
                        </details>
                        `
                            : ""
                        }
        ...
        const stepStatus =
          step.status === "passed"
            ...
            : step.status === "failed"
              ...
              : "⏭️";
        const hasContent =
          step.consoleLogs?.length || step.screenshot || step.error;
        const expandable = hasContent ? "expandable" : "";

        html += `
                        <div class="step-item-container ${step.status === "failed" ? "failed" : ""}">
                            <div class="step-item ${expandable}" onclick="toggleStepDetails(this, 'step-details-${test.id}-${idx}')">
                                ${hasContent ? `<span class="step-expand-icon">▶</span>` : '<span style="width: 20px;"></span>'}
                                <div class="step-icon ${step.status}">${stepStatus}</div>
                                <div class="step-name">${this.escapeHtml(step.title)}</div>
                                <div class="step-time">${step.duration}ms</div>
                            </div>
                            ${
                              hasContent
                                ? `
                            <div id="step-details-${test.id}-${idx}" class="step-details" style="display: none;">
                                <div class="step-meta">
                                    <div class="step-meta-item">
                                        <span>Duration:</span> ${step.duration}ms
                                    </div>
                                    <div class="step-meta-item">
                                        <span>Time:</span> ${step.startTime}
                                    </div>
                                </div>
                                ${
                                  step.consoleLogs &&
                                ...
                                <div class="step-console">
                                    <div class="step-console-header">📝 Console Logs</div>
                                ...
                                <div class="step-screenshot">
                                    ...
                                    <img src="${step.screenshot}" class="step-screenshot-img" alt="Step Screenshot" onclick="event.stopPropagation(); var modal = document.getElementById('screenshotModal'); if (modal) { modal.classList.add('active'); document.getElementById('modalImage').src = this.src; }">
                                </div>
                                `
                                    : ""
                                }
                                ...
                                <div class="step-error">
                                    ...
                                    <div class="step-stack-trace">
                                        <div class="step-stack-header">⚠️ Stack Trace</div>
                ...
                <div class="section-content">
                    <div class="screenshots-grid">
                        ...
                        <div class="screenshot-item">
                            <img src="${ss.path}" class="screenshot-preview" alt="${this.escapeHtml(ss.name)}" onclick="var modal = document.getElementById('screenshotModal'); if (modal) { modal.classList.add('active'); document.getElementById('modalImage').src = this.src; }">
                            <div class="screenshot-name">${this.escapeHtml(ss.name)}</div>
                        </div>
                        `,
                          )
                          .join("")}
    ...
    if (test.trace) {
      html += `
            <div class="detail-section">
                ...
                <div class="section-content">
                    <a href="${test.trace}" class="trace-download" target="_blank">📥 Download Trace</a>
                </div>
            </div>`;
    }
            ...
            <div class="detail-section">
                ...
                <div class="section-content">
                    ...
                    <div class="video-link">
                        <a href="${test.video}" target="_blank">📥 Download Video</a>
                    </div>
                </div>
            </div>`;
    }
  ...
  private getStyles(): string {
    return `
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .col-file { min-width: 140px; font-family: monospace; font-size: 11px; color: var(--gray-500); }
