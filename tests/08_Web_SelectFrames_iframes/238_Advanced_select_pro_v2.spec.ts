...
test("Basic Web Test - Verify Page Title", async ({ page }) => {
  ...
  await expect(page.locator("#rs-single .tta-rs__single-value")).toHaveText(
    "Playwright",
  );
  ...
  await multi
    .locator(
      '.tta-rs__multi-value:has-text("Pytest") .tta-rs__multi-value__remove',
    )
    .click();
  ...
  await expect(
    page.locator("#rs-creatable .tta-rs__multi-value", {
      hasText: "chaos-engineering",
    }),
  ).toBeVisible();
