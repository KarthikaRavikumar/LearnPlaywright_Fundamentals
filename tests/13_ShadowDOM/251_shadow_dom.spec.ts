...
test.describe("Shadow handling", () => {
  ...
  test("locate Shadow DOM and assert visible", async ({ page }) => {
    ...
    await card
      ...
      .fill("student@thetestingacademy.com");
    await card.locator('input[name="password"]').fill("pw");
    await card.getByTestId("card-account-submit").click();
    await expect(page.getByTestId("card-account-status")).toContainText(
      "student@thetestingacademy.com",
    );
    ...
    await page
      .getByTestId("card-inside-email")
      .fill("pramod@thetestingacdemy.com");
