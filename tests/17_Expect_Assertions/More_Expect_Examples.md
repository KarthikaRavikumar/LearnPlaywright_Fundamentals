...
test("toBeAttached detached", async ({ page }) => {
  ...
  await expect(page.locator(".todo-list li").first()).toBeAttached({
    attached: false,
  });
});
```
...
test("toHaveClass", async ({ page }) => {
  ...
  await expect(page.locator(".todo-list li").first()).toHaveClass(/todo/);
});
```
...
test("toHaveRole", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Get started" })).toHaveRole(
    "link",
  );
});
```
...
test("toHaveScreenshot", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 1 })).toHaveScreenshot(
    "hero-heading.png",
  );
});
```
...
test("todo flow", async ({ page }) => {
  ...
  await expect(items.first()).toContainText("Master Playwright");
});
```
